import type { NextApiRequest, NextApiResponse } from 'next';
import { checkAndEndExpiredSessions } from '../../utils/sessionExpiration';

/**
 * API endpoint to manually trigger the session expiration check
 * This can be used to check for and end expired sessions on demand
 */
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse
) {
  // Only allow POST requests
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    // Check for API key in header for security (optional)
    const apiKey = req.headers['x-api-key'];
    const configuredApiKey = process.env.SESSION_EXPIRATION_API_KEY;
    
    // If API key is configured, require it
    if (configuredApiKey && apiKey !== configuredApiKey) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    
    // Run the expiration check
    const endedCount = await checkAndEndExpiredSessions();
    
    // Return success response
    return res.status(200).json({
      success: true,
      message: `Checked for expired sessions. Ended ${endedCount} sessions.`,
      endedCount
    });
  } catch (error) {
    console.error('Error in check-session-expiration API:', error);
    return res.status(500).json({
      error: 'Failed to check for expired sessions',
      details: error instanceof Error ? error.message : String(error)
    });
  }
}
