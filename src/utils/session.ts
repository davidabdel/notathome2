...
export const endSession = async (sessionId: string, client?: SupabaseClient): Promise<boolean> => {
  try {
    ...
    // @ts-ignore - Supabase client is imported from utils or passed in
    const { error: txError } = await supabaseClient.rpc('end_session', {
    ...
    if (txError) {
      ...
      try {
        ...
        const { error: addressesDeleteError } = await supabaseClient
        ...
        const { error: sessionDeleteError } = await supabaseClient
          .from('sessions')
          .delete()
          .eq('id', sessionId);
    ...
    const { data: verifySession, error: verifyError } = await supabaseClient
      .from('sessions')
      .select('id')
      .eq('id', sessionId)
      .single();
